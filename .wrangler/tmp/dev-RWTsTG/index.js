...
// .wrangler/tmp/bundle-Y2GuXd/checked-fetch.js
...
var getStockPrice = /* @__PURE__ */ __name(async (c) => {
  ...
  try {
    ...
    return c.json({ error: true, message: error.message }, 500);
  }
}, "getStockPrice");
var getAllIndices = /* @__PURE__ */ __name(async (c) => {
  try {
    console.log("Fetching all indices");
    let data = await (await customFetch("https://www.nseindia.com/", {
      method: "POST"
    })).json();
    console.log(data);
    console.log("Fetching all indices");
    ...
    return c.json(
      { error: true, message: error.message, code: 500 },
      500
    );
  }
}, "getAllIndices");
var getIpoDetails = /* @__PURE__ */ __name(async (c) => {
  const symbol = c.req.param("symbol");
...
var timeoutMiddleware = /* @__PURE__ */ __name(async (c, next) => {
  const timeoutMs = 3e4;
  const timeoutPromise = new Promise((_, reject) => {
    setTimeout(() => reject(new TimeoutError()), timeoutMs);
  });
...
// .wrangler/tmp/bundle-Y2GuXd/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Y2GuXd/middleware-loader.entry.ts
